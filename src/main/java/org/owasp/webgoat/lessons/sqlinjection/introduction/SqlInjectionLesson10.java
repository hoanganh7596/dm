            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                ...
                if (results.getStatement() != null) {
                    ...
                    if (results.first()) {
                        ...
                        // Không có dữ liệu, kiểm tra bảng có tồn tại không
                    ...
                    // Nếu không có statement, kiểm tra bảng có tồn tại không
                    if (tableExists(connection)) {
                        return failed(this)
                                .feedback("sql-injection.10.entries")
                                .output(output.toString())
                ...
                if (tableExists(connection)) {
                    return failed(this)
                            .output("<span class='feedback-negative'>" + e.getMessage() + "</span><br>" + output.toString())
                            .build();
                } else {
                    return success(this).feedback("sql-injection.10.success").build();
                }
