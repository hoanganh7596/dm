...
public class CommentsCache {
  ...
  protected Comment parseXml(String xml) throws XMLStreamException, JAXBException {
      // Tạo một instance của JAXBContext cho lớp Comment
      ...
      // Tắt việc truy cập vào DTD và schema bên ngoài
      if (webSession.isSecurityEnabled()) {
          xif.setProperty(XMLConstants.ACCESS_EXTERNAL_DTD, ""); // Không cho phép truy cập DTD
          xif.setProperty(XMLConstants.ACCESS_EXTERNAL_SCHEMA, ""); // Không cho phép truy cập Schema
      }

      // Tạo XMLStreamReader từ chuỗi XML
      var xsr = xif.createXMLStreamReader(new StringReader(xml));

      // Tạo unmarshaller
      var unmarshaller = jc.createUnmarshaller();

      // Thực hiện unmarshalling và trả về đối tượng Comment
